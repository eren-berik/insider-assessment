...
func (r Service) Save(ctx context.Context, msg message.Cache) error {
	p, err := r.serializer.Serialize(msg)
	if err != nil {
		return err
	}
	if _, err = r.redis.Set(ctx, cacheKey(msg.ID()), p, -1).Result(); err != nil {
		return err
	}
	return nil
}
